...
import { ArrowLeft, Eye, Share2, X, Trophy, Calendar, ChevronRight, MessageCircle, Crown, Tv, Minimize2, Maximize2, MessageSquare, Shield, Activity, Clock, MapPin, Zap, Target, Bell } from 'lucide-react';
...
const PublicLiveStreamPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleViewerCountUpdate = (data: { streamId: string; count: number }) => {
      ...
      if (now - lastViewerCountUpdateRef.current >= VIEWER_COUNT_THROTTLE_MS) {
        ...
        setCurrentViewerCount(prev => {
          // ✅ SMOOTHING: Ignorar quedas bruscas >50% (reconexão do Socket.io)
          if (prev > 0 && data.count < prev * 0.5 && data.count > 0) {
            console.warn(`⚠️ Viewer count: ignorando queda brusca ${prev} → ${data.count}`);
            return prev; // Manter anterior
          }
          return data.count;
        });
      }
    ...
    // ✅ Abrir modal VIP quando admin lança promoção
    const handleVipPromo = (data: { streamId: string }) => {
      if (data.streamId !== streamId) return;
      // Só mostra para usuários logados que não são VIP
      if (user && !isVip) {
        setShowVipModal(true);
        toast('💎 Oferta VIP especial! Assine agora!', {
          duration: 5000,
          icon: '👑',
    ...
    on('vip-promo-launched', handleVipPromo);

    return () => {
      off('stream-updated', handleStreamUpdate);
      off('viewer-count-updated', handleViewerCountUpdate);
      off('vip-promo-launched', handleVipPromo);
      leaveStream(streamId);
    };
  }, [streamId, socket, isConnected, joinStream, leaveStream, on, off, user, isVip]);
                ...
                <div className="relative w-full h-full flex">
                  ...
                  {stream.is_active && (
                    <>
                      ...
                      {isFullscreen && !isMobile && isChatOpen && (
                      ...
                      {isMobile && (
                        <>
                          <MobileLiveControls
                            ...
                            onVisibilityChange={(visible) => setShowControls(visible)}
                            onChatToggle={() => {
          ...
          <div className="fixed right-0 top-0 bottom-0 w-full sm:w-[500px] bg-black/95 backdrop-blur-md border-l border-white/10 z-[9999] flex flex-col shadow-2xl">
            <div className="flex-1 overflow-hidden flex flex-col">
              <div className="flex-1 min-h-0 h-full">
                <ChatSlot id="public-overlay-chat" priority={80} className="h-full" showHeader={true} onClose={() => setIsChatOpen(false)} isActive={stream.is_active} />
              </div>
            </div>
          </div>
        )}
