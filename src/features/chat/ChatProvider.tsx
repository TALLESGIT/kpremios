...
export function ChatProvider({ children }: ChatProviderProps) {
  ...
  const getChatSession = (streamId: string): ChatSessionStore | null => {
    ...
    // ✅ GARANTIR QUE ESTÁ NA SALA: Quando solicitar sessão, reforçar entrada no socket global
    // Isso resolve casos onde a página desmontou mas o ChatHost ainda precisa dos dados
    if (socket.isConnected) {
  ...
  const sendMessage = (streamId: string, message: string, options: {
    ...
    if (!store || !socket.isConnected) {
      console.warn('⚠️ [ChatProvider] Erro ao enviar: Socket offline ou store inexistente p/', streamId);
      return;
    }

    const safeMessage = typeof message === 'string' ? message : '';
    console.log(`💬 [ChatProvider] Enviando mensagem p/ stream ${streamId}:`, safeMessage.substring(0, 20) + '...');
