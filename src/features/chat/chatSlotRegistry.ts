...
export function getActiveSlot(): ChatSlot | null {
  ...
  for (const slot of slots.values()) {
    ...
    if (!slot.container || !(slot.container instanceof Node) || !document.contains(slot.container)) {
      continue;
    }
...
export function getAllSlots(): ChatSlot[] {
  return Array.from(slots.values()).filter(slot =>
    slot.container && (slot.container instanceof Node) && document.contains(slot.container)
  );
}
