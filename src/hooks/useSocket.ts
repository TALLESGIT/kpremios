...
export function useSocket(options: UseSocketOptions = {}): UseSocketReturn {
  ...
  useEffect(() => {
    ...
    if (socketRef.current.connected) {
      if (isSocketDebug()) console.log('👥 [useSocket] Entrando na stream room via useEffect:', streamId);
      socketRef.current.emit('join-stream', { streamId });
    } else {
      // Se não estiver conectado, esperar conexão
      const handleConnect = () => {
        if (isSocketDebug()) console.log('👥 [useSocket] Conectado! Agora entrando na stream room:', streamId);
        socketRef.current?.emit('join-stream', { streamId });
      };

      if (isSocketDebug()) console.log('👥 [useSocket] Socket não conectado. Aguardando "connect" para entrar na stream:', streamId);
  ...
  const joinStream = useCallback((streamIdToJoin: string) => {
    if (!socketRef.current) {
      if (isSocketDebug()) console.warn('⚠️ [useSocket] joinStream falhou: Socket não existe');
      return;
    }

    if (isSocketDebug()) console.log('👥 [useSocket] joinStream (manual):', streamIdToJoin, 'Connected:', socketRef.current.connected);
  ...
  const leaveStream = useCallback((streamIdToLeave: string) => {
    ...
    if (isSocketDebug()) console.log('👋 [useSocket] leaveStream (manual):', streamIdToLeave);
