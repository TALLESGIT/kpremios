    ...
    if (effectiveHlsUrl) {
      return (
        <HLSViewer
          ...
          isAdmin={isAdmin}
          onError={() => {
            console.warn('⚠️ HLS falhou, exibindo tela de aguardo...');
          }}
        />
      );
    }

    // Fallback final: tela de aguardo elegante (MediaMTX — sem Agora/ZKViewer)
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-950">
        <div className="flex flex-col items-center space-y-6 animate-in fade-in duration-700">
          <div className="relative">
            <div className={`w-16 h-16 border-4 border-blue-500/20 border-t-blue-500 rounded-full ${isActuallyLive ? 'animate-spin' : ''}`} />
          ...
          <div className="text-center space-y-2 px-8">
            <h3 className="text-white font-bold tracking-widest uppercase text-sm">
              {isAdmin ? "Visualização Prévia" : "📡 Aguardando Transmissão"}
            </h3>
            <p className="text-zinc-500 text-xs font-medium uppercase tracking-wider">
              {isAdmin ? "O sinal ainda não foi iniciado no OBS/Studio" : "O sinal ainda não foi iniciado. Aguarde! ✨"}
            </p>
          </div>
        </div>
      </div>
