...
async function countEverything() {
  const tables = ['users', 'match_pools', 'pool_bets', 'raffles', 'numbers', 'extra_number_requests', 'draw_results', 'vip_subscriptions'];
  for (const table of tables) {
    const { data, count, error } = await supabase.from(table).select('*', { count: 'exact' });
    if (error) console.log(`${table}: Error ${error.message}`);
    else console.log(`${table}: ${count} rows (data length: ${data.length})`);
  }
}
countEverything();
